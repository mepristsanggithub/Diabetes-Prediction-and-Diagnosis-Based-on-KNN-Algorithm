...
%% Classifier
K_Max=50;
acc_avg_history=[]; % store accuracy for all K values
for k=1:K_Max
    ...
    for i=1:N % loop N times; each time use one fold as test set, rest as training set
        test = (indices == i);
        train = ~test;
...
function label1=knn(test_set,train_set,k)
    [n ,column1]=size(test_set); % get number of rows and columns of test set
    [m,column]=size(train_set); % get number of rows and columns of training set
        distance1=[];
        distance2=[];
            for i=1:n
                distance1(i,:)=sum((repmat(test_set(i,:),m,1)-train_set(:,1:(column-1))).^2, 2); % compute distance from test sample to all training samples
            end
            label=[]; % store labels of K nearest neighbors
            weight1=[]; % store first-layer weights
            weight2=[]; % store second-layer weights
            X=sort(distance1,2); % sort distances from test samples to all training samples
            for i=1:n
                [a,b]=sort(distance1(i,:)); % rank distances from test sample to all training samples
                for j=1:k
                ...
                X1=sort(distance2,2); % sort distances from training samples to all other training samples
                set2=X1(:,1:k);
                for z=1:k
                    [ma]=max(max(distance2(z,:)));
                    distance3=sqrt(sum((test_set(i,:).^-1-train_set(b(j),1:(column-1))).^2)); % compute distance from test sample to this training sample
                    if ma>=distance3 % if the farthest of the K neighbors of this training sample is closer than the test sample, assign second-layer weight
                        weight2(i,z)=100;
                    else
                        weight2(i,z)=0;
                    end
